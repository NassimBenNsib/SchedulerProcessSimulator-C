...
void start_algorithm(emulator * emulator) {
  ...
  process * current_process = NULL, * highest_priority_process = NULL;
  puts("Static Priority Preemptive Scheduling");
  ...
  while (!is_list_process_empty(emulator -> list_incoming_process) || !is_list_process_empty(emulator -> list_waiting_process)) {
    ...
    if (is_list_process_empty(emulator -> list_waiting_process)) {
      ...
      highest_priority_process = get_highest_priority_process(emulator -> list_waiting_process);
      if (current_process == NULL || highest_priority_process != current_process) {
        if (current_process != NULL && highest_priority_process != current_process) {
...
void start_emulator(int number_arguments, char * arguments[]) {
  ...
  set_algorithm_name( & emulator, "Static Priority Preemptive Scheduling");
